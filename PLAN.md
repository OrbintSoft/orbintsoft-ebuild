...
- [~] **3.7** (per-package) Convert live `-9999` → versioned ebuilds where upstream has
      **stable** tags; decided dep by dep (live stays where there are none). Census
      ...
      SRC_URI-less live ebuilds, 3.5): only `dev-util/fnm` (v1.39.0) and `dev-libs/tvision`
      (v2.0) have clean stable tags; `dev-util/shellcheck` (v0.11.0) → Phase 7.1; the rest
      stay live (no tags: pamtester, ssh-profile-config, bt-keys-sync, polo; betas only:
      fsearch; dirty `+claude` versioning / binary repackage: claude-desktop; stub: ksshaskpass).
  ...
  - [ ] **tvision** 9999 → 2.0 — cmake (mechanically simple); pending — turbo (4.2) depends on
        tvision and magiblot often wants HEAD, so confirm before converting.
